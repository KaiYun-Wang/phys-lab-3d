...
-- Date: 2026-07-19 09:31:17
-- Generated by DBX
--
-- 前置：PostgreSQL 镜像需带 pgvector（推荐 pgvector/pgvector:pg16）
...
CREATE SEQUENCE IF NOT EXISTS "public"."experiment_views_id_seq"
  AS bigint
  START WITH 1
  INCREMENT BY 1
  MINVALUE 1
...
COMMENT ON TABLE "public"."kb_documents" IS 'RAG 知识库文档元数据';
COMMENT ON COLUMN "public"."kb_documents"."id" IS '文档 ID，自增主键';
COMMENT ON COLUMN "public"."kb_documents"."title" IS '知识库文档标题';
COMMENT ON COLUMN "public"."kb_documents"."filename" IS '原始文件名';
COMMENT ON COLUMN "public"."kb_documents"."content_type" IS 'MIME 类型，如 application/pdf、text/plain';
COMMENT ON COLUMN "public"."kb_documents"."status" IS '处理状态：PROCESSING / READY / FAILED';
COMMENT ON COLUMN "public"."kb_documents"."chunk_count" IS '切片数量';
COMMENT ON COLUMN "public"."kb_documents"."create_time" IS '创建时间，插入时自动填充';
COMMENT ON COLUMN "public"."kb_documents"."update_time" IS '更新时间，插入/更新时自动填充';
...
COMMENT ON TABLE "public"."subject_types" IS '物理子学科分类（力学、光学等）';
...
COMMENT ON TABLE "public"."kb_chunks" IS '知识库文档切片及向量嵌入';
COMMENT ON COLUMN "public"."kb_chunks"."id" IS '切片 ID，自增主键';
...
COMMENT ON COLUMN "public"."kb_chunks"."embedding" IS '向量嵌入（pgvector，维度 1024）';
COMMENT ON COLUMN "public"."kb_chunks"."create_time" IS '创建时间，插入时自动填充';
...
COMMENT ON COLUMN "public"."ai_chat_messages"."rag_hit_count" IS 'assistant 回复时知识库命中条数；user/system 为空';
...
COMMENT ON TABLE "public"."experiment_comments" IS '实验评论与楼中楼回复';
...
CREATE TABLE "public"."experiment_views" (
  "id" bigint NOT NULL DEFAULT nextval('experiment_views_id_seq'::regclass),
  "experiment_id" bigint NOT NULL,
  "user_id" bigint,
...
COMMENT ON TABLE "public"."experiment_views" IS '实验详情页访问日志（每次打开记一行）';
COMMENT ON COLUMN "public"."experiment_views"."id" IS '访问记录 ID，自增主键';
COMMENT ON COLUMN "public"."experiment_views"."experiment_id" IS '实验 ID，逻辑关联 experiments(id)';
COMMENT ON COLUMN "public"."experiment_views"."user_id" IS '登录用户 ID，逻辑关联 users(id)；游客为空';
COMMENT ON COLUMN "public"."experiment_views"."viewed_at" IS '访问时间';
CREATE INDEX "idx_experiment_views_experiment_viewed" ON "public"."experiment_views" USING btree ("experiment_id", "viewed_at");
CREATE INDEX "idx_experiment_views_viewed_at" ON "public"."experiment_views" USING btree ("viewed_at");
...
COMMENT ON TABLE "public"."experiments" IS '3D 物理实验内容与冗余统计字段';
...
COMMENT ON COLUMN "public"."experiments"."visitor_count" IS '访客数（冗余统计，预留）';
COMMENT ON COLUMN "public"."experiments"."favorite_count" IS '收藏数（冗余统计）';
COMMENT ON COLUMN "public"."experiments"."view_count" IS '浏览量（冗余统计，与 experiment_views 同步累加）';
COMMENT ON COLUMN "public"."experiments"."comment_count" IS '评论数（冗余统计）';
...
COMMENT ON TABLE "public"."ai_chat_sessions" IS 'AI 辅导会话';
...
COMMENT ON TABLE "public"."admins" IS '管理端账号';
...
COMMENT ON TABLE "public"."experiment_comment_likes" IS '实验评论点赞记录';
...
COMMENT ON TABLE "public"."users" IS '用户端账号';
...
COMMENT ON COLUMN "public"."users"."status" IS '账号状态：ENABLED 正常 / DISABLED 禁用（封号）';
...
COMMENT ON TABLE "public"."experiment_favorites" IS '用户收藏实验记录';
